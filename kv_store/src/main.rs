...
fn main() {
    // we create our source data. It's a list of tuples containing a "Key" (ID) and a "Value" (the actual data converted into bytes)
    ...
    let mut file = std::fs::File::create_new("storage.db").expect("Failed to create file");
